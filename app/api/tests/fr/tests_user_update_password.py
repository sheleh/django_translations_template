...
class UserPasswordChangeTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.user = User.objects.create(email='test@test.com', language='fr')
    ...
    def test_user_can_not_change_password_without_current_password_field(self):
        ...
        self.assertEqual(response.data['current_password'],
                         [ErrorDetail(string='Ce champ ne peut être vide.', code='blank')])
    ...
    def test_user_can_not_change_password_with_not_matched_new_passwords(self):
        ...
        self.assertEqual(set(response.data.keys()), expected_error_key)
        self.assertEqual(response.data['new_password'],
                         [ErrorDetail(string='Ces mots de passe ne correspondent pas', code='invalid')])
    ...
    def test_user_can_not_change_password_with_same_current_and_new_passwords(self):
        ...
        self.assertEqual(set(response.data.keys()), expected_error_key)
        self.assertEqual(response.data['new_password'],
                         [ErrorDetail(string='Les mots de passe ne doivent pas être les mêmes.', code='invalid')])
    ...
    def test_user_can_change_password(self):
        ...
        self.assertEqual(response.data['success'], 'Votre mot de passe a été changé avec succès !')
        self.assertEqual(mail.outbox[0].subject, 'Le mot de passe a été changé !!!')
